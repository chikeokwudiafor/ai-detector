...
from detection_test import AIDetectorTest, get_result_classification
...
def process_file(file, file_type, filename="unknown"):
    ...
    try:
        if file_type == "image":
            result_type, confidence, raw_scores = AIDetectorTest.detect_image(file, filename)
        elif file_type == "text":
...
def index():
    """Main route handler - TEST VERSION"""
    result = None
    confidence = None
    result_class = None
    result_icon = None
...
def submit_feedback():
    ...
    try:
        ...
        # Save feedback with test marker
        ...
        if success:
            return jsonify({"message": "Thank you for your feedback! (Test mode)"})
        else:
